...
typedef struct RMQ{
    ...
    int change(int pos, int node, int nodeLeft, int nodeRight, int value){
        ...
        rangeMin[node] = min(rangeMin[node], value);
        int mid = (nodeLeft + nodeRight) / 2;
        return min(change(pos, node*2, nodeLeft, mid, value), change(pos, node*2+1, mid+1, nodeRight, value));
    }
    int change(int pos, int node, int value){
